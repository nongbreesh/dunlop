...
<aside class="right-side">
    ...
    <section class="content-header">
        <h1>
            News  <a href="javascript:;" onclick="add_product()" class="add-new-h2">Add News</a>
    ...
    <div class="modal-dialog  modal-lg">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title"><i class="fa fa-shopping-cart"></i> Add News</h4>
            </div>
...
<script>
    $(document).ready(function() {
        ...
        $("#form_add_product").on('submit', function(e) {
            ...
            if (is_update == 'false') {
                $.ajax({
                    url: "<?php echo base_url(); ?>" + "index.php/service/add_news",
                    type: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                ...
                $.ajax({
                    url: "<?php echo base_url(); ?>" + "index.php/service/edit_news/" + input_id,
                    type: "POST",
                    data: $(this).serialize(),
                    dataType: "json",
                    success: function(data) {
    ...
    function edit_product(id) {
        ...
        if (id != "") {
            $.ajax({
                type: "POST",
                url: "<?php echo base_url(); ?>" + "index.php/service/getnews_detail",
                data: {"id": id},
                dataType: 'json',
                success: function(data) {
                    console.log(data);
                    $('#input_headine').val(data.result.NEWS_Headline);
                    $('#input_url').val(data.result.NEWS_URL);
                   
                    $('#product_pic').html('<img src="../public/uploads/Thumbnails_' + data.result.NEWS_IMG + '" height="50"><input type="hidden" id="input_hdimage" name="input_hdimage" value="' + data.result.NEWS_IMG + '" />');
    ...
    function removedata(id) {
        if (confirm('ต้องการลบข้อมูลหรือไม่?')) {
            $.ajax({
                type: "POST",
                url: "<?php echo base_url(); ?>" + "index.php/service/delete_news",
                data: {'id': id},
                dataType: "json",
                success: function(data) {
    ...
    function update_list() {
        ...
        setTimeout(
                function()
                {
                    $.ajax({
                        type: "POST",
                        url: "<?php echo base_url(); ?>" + "index.php/service/load_news",
                        dataType: "html",
                        success: function(data) {
