...
class user_model extends CI_Model {
    ...
    function user_login($user_id = '', $password = '', $remember_me = '') {
        $query = $this->db->query("SELECT * FROM dunlop_users where user_login = '" . $user_id . "'  and user_pass = '" . $password . "'  LIMIT 1");
        if ($query->num_rows() > 0) {
            $row = $query->row();
            ...
            $query2 = $this->db->query("SELECT umeta_id,meta_value FROM dunlop_usermeta  where user_id = '" . $row->ID . "' AND meta_key = 'user_logindate'");
            $query3 = $this->db->query("SELECT umeta_id,meta_value FROM dunlop_usermeta  where user_id = '" . $row->ID . "' AND meta_key = 'user_logoutdate'");
            $row2 = $query2->row();
            $row3 = $query3->row();
            ...
            $this->db->update('dunlop_usermeta', $row);
    ...
    function get_by_id($id) {
        $this->db->where('ID', $id);
        return $this->db->get('dunlop_users');
    }
